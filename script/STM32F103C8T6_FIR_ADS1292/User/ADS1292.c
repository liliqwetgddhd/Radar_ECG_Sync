...
{
	CS_L;				// 片选拉低
  SPI2_ReadWriteByte(addr);	// 包含命令操作码和寄存器地址
  Delay_1us(10);
  SPI2_ReadWriteByte(0x00);	// 要读取的寄存器数+1
  Delay_1us(10);
  SPI2_ReadWriteByte(data);	// 写入的数据
	Delay_1us(10);
	CS_H;				// 片选置高
}
...
u8 ADS1292_Read_Reg(u8 addr)
{
  ...
  SPI2_ReadWriteByte(0x00); 			// 要读取的寄存器数+1
  Delay_1us(10);
  Rxdata = SPI2_ReadWriteByte(0xFF); 	// 读取的数据
	Delay_1us(10);
	CS_H;					// 片选置高
  return Rxdata;
}
...
void ADS1292_PowerOnInit(void)
{
  ...
  Delay_50ms(20); // 等待稳定，可以开始使用ADS1292R
	...
	CS_L;
	Delay_1us(10);
  SPI2_ReadWriteByte(SDATAC); // 发送停止连续读取数据命令
	Delay_1us(10);
	CS_H;
